...
final filteredActivitiesProvider =
    Provider<AsyncValue<List<ActivityListItem>>>((ref) {
  ...
  return activitiesAsync.whenData((items) {
    var filtered = items.where((item) {
      final matchesSearch = filter.searchQuery.isEmpty ||
          ...
          (item.activity.category?.toLowerCase().contains(
              ...
              false);
      final matchesCategory = filter.category == null ||
    ...
    filtered = switch (filter.sort) {
      ActivitySort.recentlyDone => filtered
        ..sort((a, b) {
          final aDate = a.item.lastDoneAt ?? DateTime.fromMillisecondsSinceEpoch(0);
          final bDate = b.item.lastDoneAt ?? DateTime.fromMillisecondsSinceEpoch(0);
          return bDate.compareTo(aDate);
        }),
      ActivitySort.overdue => filtered
        ..sort((a, b) {
      ...
      ActivitySort.alphabetical => filtered
        ..sort((a, b) =>
            a.item.activity.title.compareTo(b.item.activity.title)),
    };
